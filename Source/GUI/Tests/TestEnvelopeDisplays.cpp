#include "TestEnvelopeDisplays.h"
...
class TestEnvelopeDisplays::EnvelopeDisplayScalePanel : public juce::Component
{
public:
    EnvelopeDisplayScalePanel(float scale,
                              const juce::String& scaleLabelText,
                              const TSS::EnvelopeDisplayLook& displayLook,
        ...
        , displayLook_(displayLook)
    {
        ...
        display_ = std::make_unique<TSS::EnvelopeDisplay>(dimensions_, displayLook_);
        display_->setUiScale(scale_);
        display_->setDelay(5, false);
        display_->setAttack(20, false);
        display_->setDecay(30, false);
        display_->setSustain(50, false);
        display_->setRelease(25, false);
...
private:
    ...
    TSS::EnvelopeDisplayLook displayLook_ {};
    std::unique_ptr<TSS::Label> scaleLabel_;
    std::unique_ptr<TSS::EnvelopeDisplay> display_;
};

TestEnvelopeDisplays::TestEnvelopeDisplays(TSS::ISkin& skin, const DisplayBandDimensions& dimensions)
    : dimensions_(dimensions)
{
    setSkin(skin);
}

TestEnvelopeDisplays::~TestEnvelopeDisplays() = default;

void TestEnvelopeDisplays::setSkin(TSS::ISkin& skin)
{
    skin_ = &skin;
    rebuildPanels();
}

void TestEnvelopeDisplays::resized()
{
    layoutColumnPanels();
}

int TestEnvelopeDisplays::getPreferredWidth() const
{
...
int TestEnvelopeDisplays::getPreferredHeight() const
{
    int maxHeight = 0;
...
void TestEnvelopeDisplays::rebuildPanels()
{
    if (skin_ == nullptr)
        return;
    ...
    const auto displayLook = TSS::envelopeDisplayLookFromSkin(*skin_);
    ...
    {
        auto panel = std::make_unique<EnvelopeDisplayScalePanel>(
            spec.scale,
            spec.label,
            displayLook,
            labelLook,
...
void TestEnvelopeDisplays::layoutColumnPanels()
{
    const int panelGap = TestScaleColumns::kGap * TestScaleColumns::kPanelGapMultiplier;
    int nextColumnX = 0;
